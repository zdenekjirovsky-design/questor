...
// exponenciálně (5 s → 10 s → … → max 5 min).
import type { ProgresStudenta, TestVysledek } from '@questor/sdilene';
import { vychoziUloziste, type QuestorKlient, type Uloziste } from './klient';
...
export class SyncFronta {
  ...
  /**
   * Zkusí odeslat frontu v pořadí. Při prvním selhání se zastaví, položka
   * zůstává a naplánuje se exponenciální odklad. NIKDY nevyhazuje — selhání
   * sítě je tiché.
   */
  async odesli(
